...
if (trim((string) $mailSentStatus) !== '') {
  ...
  $query = $query . " AND ((vl.result_status = 7 AND vl.hbsag_result is NOT NULL AND vl.hbsag_result !='') OR (vl.result_status = 4 AND (vl.hbsag_result is NULL OR vl.hbsag_result = '')))";
}
      ...
      <select id="sample" name="sample[]" multiple="multiple" class="search isRequired"
        ...
        foreach ($result as $sample) {
          if (trim((string) $sample['sample_code']) !== '') {
            ?>
            <option value="<?php echo $sample['hepatitis_id']; ?>">
