				...
				<div class="box filter-panel filter-panel-collapsed">
					...
					<div class="filter-actions">
						...
						<?php if (_isAllowed("/hepatitis/requests/hepatitis-add-request.php") && !$hidesrcofreq) { ?>
							<a style=" margin: 0px 5px; " href="/hepatitis/requests/hepatitis-add-request.php" class="filter-keep btn btn-primary btn-sm pull-right">
								<em class="fa-solid fa-plus"></em> <?= _htmlTranslate("Add new Hepatitis Request"); ?>
							</a>
						<?php } ?>
						<?php if (_isAllowed("/hepatitis/requests/export-hepatitis-requests.php")) { ?>
							<a class="filter-export btn btn-success btn-sm pull-right" href="javascript:void(0);" onclick="exportAllPendingHepatitisRequest();">
	...
	<?php if ($general->isLISInstance()) { ?>
		...
		function forceResultSync(sampleCode) {
			...
			if (remoteSync && remoteURL != null && remoteURL != '') {
				var jqxhr = $.ajax({
					url: "/tasks/remote/results-sender.php?sampleCode=" + encodeURIComponent(sampleCode) + "&forceSyncModule=hepatitis",
				})
					.done(function (data) {
						////console.log(data);
						//alert( "success" );
