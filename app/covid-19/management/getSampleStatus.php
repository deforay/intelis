...
$tQuery = "SELECT COUNT(sample.covid19_id) as total, status.status_id, status.status_name
        FROM form_covid19 AS sample
        $joins
...
$vlSuppressionQuery = "SELECT COUNT(sample.covid19_id) as total,
...
$tat = $covid19Service->getTurnaroundTimeSeries(
    conditions: $filters,
    params: $params,
    joins: $joins
);
...
$testReasonQuery = "SELECT COUNT(sample.sample_code) AS total, reason.test_reason_name
        FROM form_covid19 AS sample
        INNER JOIN r_covid19_test_reasons AS reason ON sample.reason_for_covid19_test = reason.test_reason_id
        LEFT JOIN batch_details AS batch ON batch.batch_id = sample.sample_batch_id
        WHERE $whereCondition
            AND sample.reason_for_covid19_test IS NOT NULL
