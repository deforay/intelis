...
$iFilteredTotal = (int) ($db->rawQueryOne("SELECT COUNT(*) AS total FROM r_covid19_sample_type $sWhere")['total'] ?? 0);
...
foreach ($rResult as $aRow) {
    ...
    $row[] = _isAllowed("covid19-sample-type.php") && $general->isLISInstance() === false ? $status : htmlspecialchars((string) ($aRow['status'] ?? ''), ENT_QUOTES, 'UTF-8');
    $output['aaData'][] = $row;
}
