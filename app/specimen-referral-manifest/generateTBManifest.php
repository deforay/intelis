...
if (trim((string) $id) !== '') {
    ...
    $sQuery = "SELECT remote_sample_code,fd.facility_name as clinic_name,fd.facility_district,vl.patient_name,vl.patient_surname,vl.is_encrypted,patient_dob,patient_age,sample_collection_date,patient_gender,patient_id,pd.manifest_code, l.facility_name as lab_name from specimen_manifests as pd Join form_tb as vl ON vl.sample_package_id=pd.manifest_id Join facility_details as fd ON fd.facility_id=vl.facility_id Join facility_details as l ON l.facility_id=vl.lab_id where pd.manifest_id IN(" . $db->inIntList($id) . ") ORDER BY remote_sample_code ASC";
