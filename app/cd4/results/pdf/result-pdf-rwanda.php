...
if (!empty($result)) {
     ...
     if ($result['crag_test_results'] != '' || $result['crag_test_results'] != NULL) {
          $crAgResult = '<br>&nbsp;&nbsp;CrAg Test Result &nbsp;&nbsp;:&nbsp;&nbsp;' . ucfirst((string) $result['crag_test_results']);
     }
     $html .= '<tr>';
     $html .= '<td colspan="3" style="line-height:10px;"></td>';
     $html .= '</tr>';
     ...
     $html .= '<td style="line-height:11px;font-size:11px;text-align:left;">' . ($result['instrument_machine_name'] ?? $result['cd4_test_platform'] ?? '') . '</td>';
     $html .= '</tr>';
     $html .= '<tr>';
     ...
     if (!empty($result['cd4_result'])) {
          ...
          if (isset($arr['vl_report_qr_code']) && $arr['vl_report_qr_code'] == 'yes' && !empty($remoteURL)) {
               ...
               $pdf->write2DBarcode($remoteURL . '/cd4/results/view.php?q=' . urlencode((string) $viewId), 'QRCODE,H', 150, 170, 30, 30, [], 'N');
          }
          $pdf->lastPage();
