...
try {
    ...
    if (!empty($input['patientId'])) {
        $patientId = implode("','", array_map($db->escape(...), (array) $input['patientId']));
        $where[] = " vl.patient_id IN ('" . $patientId . "') ";
    }

    if (!empty($input['patientName'])) {
        $where[] = " (vl.patient_name like '" . $db->escape($input['patientName']) . "' OR vl.patient_surname like '" . $db->escape($input['patientName']) . "')";
    }
    ...
    $total = (int) ($db->rawQueryOne("SELECT COUNT(*) AS total FROM form_covid19 AS vl $whereString")['total'] ?? 0);
    ...
    if ($affectedSamples !== []) {
        $resultVersions = SavedResultGuard::resultVersions($db, 'covid19', $affectedSamples);
        foreach ($rowData as &$row) {
            $row['resultVersion'] = $resultVersions[(string) ($row['covid19Id'] ?? '')] ?? null;
        }
        unset($row);
    }
...
$general->addApiTracking($transactionId, $user['user_id'], count($rowData ?? []), 'fetch-results', 'covid19', $_SERVER['REQUEST_URI'], $origJson, $payload, 'json', null, null, $authToken);
