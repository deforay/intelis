...
try {
    ...
    $total = (int) ($db->rawQueryOne("SELECT COUNT(*) AS total FROM form_tb AS vl $whereStr")['total'] ?? 0);
    ...
    if ($affectedSamples !== []) {
        $resultVersions = SavedResultGuard::resultVersions($db, 'tb', $affectedSamples);
...
$general->addApiTracking($transactionId, $user['user_id'], count($rowData ?? []), 'fetch-results', 'tb', $_SERVER['REQUEST_URI'], $origJson, $payload, 'json', null, null, $authToken);
