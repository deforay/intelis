...
try {
    ...
    $existingSamples = $testRequestsService->mapSamplesByAppSampleCode(
        'form_generic',
        [
            'sample_id', 'unique_id', 'sample_code', 'sample_code_format', 'sample_code_key',
            'remote_sample_code', 'remote_sample_code_format', 'remote_sample_code_key',
    ...
    if ($dataItems !== []) {
        ...
        foreach ($dataItems as $rootKey => $data) {
            ...
            if (!$general->checkApiFacilityAllowed((int) ($data['facilityId'] ?? 0), $apiUserFacilityMap, $user['user_id'] ?? null, 'api/generic-tests/save-request')
            ...
            if (!empty($rowData)) {
                ...
                // An earlier payload can leave a sample whose code has not been minted
                // yet -- the queue drains in batches -- and an update reads its code
                // straight off the row, so without this the response answers
                // sampleCode: null on every update until a cron happens to catch up.
                // Sending it through the same post-commit pass as this payload's
                // inserts mints it now.
            ...
            if (trim((string) ($data['labSampleCode'] ?? '')) === '') {
                unset($genericData['lab_assigned_code']);
            }
...
$general->addApiTracking($transactionId, $user['user_id'], $dataCounter, 'save-request', 'generic-tests', $_SERVER['REQUEST_URI'], $origJson, $payload, 'json', null, null, $authToken);
