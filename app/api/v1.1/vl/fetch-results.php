...
try {
    ...
    $total = (int) ($db->rawQueryOne("SELECT COUNT(*) AS total FROM form_vl AS vl $whereString")['total'] ?? 0);
    ...
    if ($affectedSamples !== []) {
        $resultVersions = SavedResultGuard::resultVersions($db, 'vl', $affectedSamples);
...
$general->addApiTracking($transactionId, $user['user_id'], count($rowData ?? []), 'fetch-results', 'vl', $_SERVER['REQUEST_URI'], $origJson, $payload, 'json', null, null, $authToken);
