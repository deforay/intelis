...
try {
    ...
    $existingSamples = $testRequestsService->mapSamplesByAppSampleCode(
        'form_eid',
        [
            'eid_id', 'unique_id', 'sample_code', 'sample_code_format', 'sample_code_key',
    ...
    foreach ($dataItems as $rootKey => $data) {
        ...
        if (!$general->checkApiFacilityAllowed((int) ($data['facilityId'] ?? 0), $apiUserFacilityMap, $user['user_id'] ?? null, 'api/eid/save-request')
        ...
        if ($enableDuplicateDetection) {
            $duplicateResult = $testRequestsService->handleDuplicateDetection(
                sampleData: $eidData,
                testType: 'eid',
                excludeSampleId: !empty($rowData) ? $rowData['eid_id'] : null,
                transactionId: $transactionId,
        ...
        $eidData['form_attributes'] = $formAttributesStr === null || $formAttributesStr === '' || $formAttributesStr === '0' ? null : $db->func($formAttributesStr);
