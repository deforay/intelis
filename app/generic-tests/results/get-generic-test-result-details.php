...
try {
     ...
     $aColumns = ['vl.sample_code', 'vl.sample_code', 'vl.remote_sample_code', 'b.batch_code', 'vl.patient_id', "CONCAT(COALESCE(vl.patient_first_name,''), COALESCE(vl.patient_middle_name,''),COALESCE(vl.patient_last_name,''))", 'f.facility_name', 'testingLab.facility_name', 'f.facility_state', 'f.facility_district', 's.sample_type_name', 'vl.result', "DATE_FORMAT(vl.last_modified_datetime,'%d-%b-%Y')", 'ts.status_name'];
     $orderColumns = ['vl.sample_code', 'vl.sample_code', 'vl.remote_sample_code', 'b.batch_code', 'vl.patient_id', "CONCAT(COALESCE(vl.patient_first_name,''), COALESCE(vl.patient_middle_name,''),COALESCE(vl.patient_last_name,''))", 'f.facility_name', 'testingLab.facility_name', 'f.facility_state', 'f.facility_district', 's.sample_type_name', 'vl.result', "vl.last_modified_datetime", 'ts.status_name'];
     if (!empty($_POST['from']) && $_POST['from'] == "enterresult") {
          $aColumns = ['vl.sample_code', 'vl.remote_sample_code', 'b.batch_code', 'vl.patient_id', "CONCAT(COALESCE(vl.patient_first_name,''), COALESCE(vl.patient_middle_name,''),COALESCE(vl.patient_last_name,''))", 'f.facility_name', 'testingLab.facility_name', 's.sample_type_name', 'vl.result', "DATE_FORMAT(vl.last_modified_datetime,'%d-%b-%Y')", 'ts.status_name'];
          $orderColumns = ['vl.sample_code', 'vl.remote_sample_code', 'b.batch_code', 'vl.patient_id', "CONCAT(COALESCE(vl.patient_first_name,''), COALESCE(vl.patient_middle_name,''),COALESCE(vl.patient_last_name,''))", 'f.facility_name', 'testingLab.facility_name', 's.sample_type_name', 'vl.result', "vl.last_modified_datetime", 'ts.status_name'];
     }
     ...
     $sWhere = [...$sWhere, ...ListingFilterClauseBuilder::clauses($db, $_POST, [
          'sampleType' => ['s.sample_type_id', ListingFilterClauseBuilder::EQUALS],
          'facilityName' => ['f.facility_id', ListingFilterClauseBuilder::INT_LIST],
          'vlLab' => ['vl.lab_id', ListingFilterClauseBuilder::INT_LIST],
          'artNo' => ['vl.patient_id', ListingFilterClauseBuilder::CONTAINS],
     ])];
     ...
     foreach ($rResult as $aRow) {
          ...
          if (isset($_POST['vlPrint'])) {
               ...
               if ($aRow['result_status'] == 7 && $aRow['locked'] == 'yes' && !_isAllowed("/generic-tests/requests/edit-locked-generic-tests-samples")) {
