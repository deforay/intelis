...
try {
    ...
    // ---------------------------------------------------------------------
    if (!empty($_POST['testResult']['labId']) && is_array($_POST['testResult']['labId'])) {
        // Multi-test (TB-style) result entry. The actual persistence (per-test rows +
        ...
        $general->activityLog(
            ...
            $_SESSION['userName'] . ' updated multi-test result for sample ' . ($_POST['sampleCode'] ?? ($_POST['requestSampleId'] ?? '')),
            'lab-test-result'
        );
    ...
    if (isset($_POST['requestSampleId']) && $_POST['requestSampleId'] != '' && ($_POST['isSampleRejected'] == 'no' || $_POST['isSampleRejected'] == '')) {
        ...
        if (!empty($_POST['testName'])) {
            ...
            // subTestResult is always set above -- to the selected sub-tests or to
            // 'default' -- so every row is keyed by its sub-test. A test type with no
            // sub-tests posts testName[][], which gives each row its own key, while
            // the test type's own fields are posted once, under key 0.
            $subTestResultIsDefault = $_POST['subTestResult'] === 'default';
            $typeField = static fn(string $field, $subTestName) => $_POST[$field][$subTestName]
                ?? ($subTestResultIsDefault ? ($_POST[$field][0] ?? null) : null);
            foreach ($_POST['testName'] as $subTestName => $subTests) {
                foreach ($subTests as $testKey => $testKitName) {
                    if (!empty($testKitName)) {
                        $testData = ['generic_id' => $_POST['requestSampleId'], 'sub_test_name' => $subTestName, 'result_type' => $typeField('resultType', $subTestName), 'test_name' => ($testKitName == 'other') ? $_POST['testNameOther'][$subTestName][$testKey] : $testKitName, 'facility_id' => $_POST['labId'] ?? null, 'sample_tested_datetime' => DateUtility::isoDateFormat($_POST['testDate'][$subTestName][$testKey] ?? '', true), 'testing_platform' => $_POST['testingPlatform'][$subTestName][$testKey] ?? null, 'kit_lot_no' => (str_contains((string)$testKitName, 'RDT')) ? $_POST['lotNo'][$subTestName][$testKey] : null, 'kit_expiry_date' => (str_contains((string)$testKitName, 'RDT')) ? DateUtility::isoDateFormat($_POST['expDate'][$subTestName][$testKey]) : null, 'result_unit' => $_POST['testResultUnit'][$subTestName][$testKey], 'result' => $_POST['testResult'][$subTestName][$testKey], 'final_result' => $typeField('finalResult', $subTestName), 'final_result_unit' => $typeField('finalTestResultUnit', $subTestName), 'final_result_interpretation' => $typeField('resultInterpretation', $subTestName)];
