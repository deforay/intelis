...
if (isset($facility) && array_filter($facility) !== []) {
	$query = $query . " AND vl.facility_id IN (" . $db->inIntList($facility) . ")";
}
...
if (trim((string) $state) !== '') {
	$query = $query . " AND f.facility_state LIKE '%" . $db->escapeLike((string) $state) . "%' ";
}
if (trim((string) $district) !== '') {
	$query = $query . " AND f.facility_district LIKE '%" . $db->escapeLike((string) $district) . "%' ";
}
...
if (trim((string) $mailSentStatus) !== '') {
	if (trim((string) $type) === 'request') {
		...
		$query = $query . " AND vl.is_result_mail_sent='" . $db->escape((string) $mailSentStatus) . "' AND vl.result IS NOT NULL AND vl.result!= ''";
	}
}
	...
	<div class="form-group">
		...
		<div class="col-lg-9">
			<div style="width:100%;margin:0 auto;clear:both;">
				<a href="#" id="select-all-sample" style="float:left" class="btn btn-info btn-xs">Select
					All&nbsp;&nbsp;<em class="fa-solid fa-chevron-right"></em></a> <a href="#" id="deselect-all-sample"
					style="float:right" class="btn btn-danger btn-xs"><em
						class="fa-solid fa-chevron-left"></em>&nbsp;Deselect All</a>
			</div><br /><br />
			<select id="sample" name="sample[]" multiple="multiple" class="search isRequired"
				title="Please select sample(s)">
				<?php
