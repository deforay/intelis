...
/**
 * The TB request, result-entry, print, failed and result-status listings, driven
 ...
 */
final class TbListingSqlEncodingTest extends TestCase
{
    private const DATABASE = 'intelis_tb_listing_sql_test';
    ...
    protected function setUp(): void
    {
        ...
        $db = LegacyAppHarness::boot(self::DATABASE . '_' . getmypid(), [
            ...
            'r_tb_sample_rejection_reasons', 'r_recommended_corrective_actions', 'r_funding_sources',
            'r_implementation_partners', 'r_tb_results', 'form_tb', 'activity_log', 'system_config',
            'global_config',
        ]);
        ...
        $db->rawQuery("INSERT INTO batch_details (batch_id, batch_code, test_type) VALUES (1, 'B-ONE', 'tb')");

        // Two accepted results with nothing printed yet, and one failed test.
        $this->seed([
            'sample_code' => 'TB001', 'patient_id' => 'P-7', 'patient_gender' => 'female',
            'sample_batch_id' => 1, 'sample_package_code' => 'MF-1',
        ]);
        $this->seed(['sample_code' => 'TB002', 'patient_id' => 'P-8', 'patient_gender' => 'male']);
        $this->seed([
            'sample_code' => 'TB003', 'patient_id' => 'P-9', 'patient_gender' => 'male',
    ...
    private function seed(array $columns): void
    {
        ...
        LegacyAppHarness::db()->rawQuery(
            "INSERT INTO form_tb (" . implode(', ', $names) . ") VALUES (" . implode(', ', $values) . ")"
        );
    }
    ...
    private function listed(string $endpoint, array $post): array
    {
        ...
        foreach ($json['aaData'] as $row) {
            foreach ($row as $cell) {
                if (preg_match('/TB00\d/', strip_tags((string) $cell), $m) === 1) {
                    $codes[] = $m[0];
                    break;
                }
            }
    ...
    {
        self::assertSame([], $this->listed('/tb/requests/get-request-list.php', [
    ...
    {
        self::assertSame(['TB001'], $this->listed('/tb/requests/get-request-list.php', [
    ...
    #[RunInSeparateProcess]
    public function testRequestListTreatsAQuoteInADecodedFundingSourceAsPartOfTheValue(): void
    {
        self::assertSame([], $this->listed('/tb/requests/get-request-list.php', [
            'fundingSource' => base64_encode('1") OR ("1"="1'), 'hidesrcofreq' => '',
        ]));
    }

    #[RunInSeparateProcess]
    public function testRequestListKeepsOnlyTheNumbersOfALabList(): void
    {
        self::assertSame([], $this->listed('/tb/requests/get-request-list.php', [
            'vlLab' => '0) OR (1=1', 'hidesrcofreq' => '',
        ]));
    }
    ...
    public function testFailedResultsKeepOnlyTheNumbersOfAStatusList(): void
    {
        ...
        self::assertSame(['TB003'], $this->listed('/tb/results/get-failed-results.php', [
            'status' => '5,0) OR (1=1',
        ]));
    }
    ...
    {
        self::assertSame([], $this->listed('/tb/results/get-failed-results.php', [
            'sSearch' => "zz' OR '1'='1",
        ]));
    }
    ...
    public function testFailedResultsDiscardAnythingButAscOrDescAsTheSortDirection(): void
    {
        self::assertSame(['TB003'], $this->listed('/tb/results/get-failed-results.php', [
            ...
            'sSortDir_0' => 'asc, (SELECT 1 FROM form_tb WHERE 1=1 UNION SELECT 2)',
        ]));
    }
    ...
    {
        self::assertSame([], $this->listed('/tb/results/get-results-for-print.php', [
    ...
    {
        self::assertSame(['TB001'], $this->listed('/tb/results/get-results-for-print.php', [
    ...
    {
        self::assertSame([], $this->listed('/tb/results/tb-samples-for-manual-result-entry.php', [
            'batchCode' => 'B-ONE" OR "1"="1',
        ]));
    }

    #[RunInSeparateProcess]
    public function testManualResultEntryStillFindsABatch(): void
    {
        self::assertSame(['TB001'], $this->listed('/tb/results/tb-samples-for-manual-result-entry.php', [
            'batchCode' => 'B-ONE',
        ]));
    }
    ...
    {
        self::assertSame(['TB003'], $this->listed('/tb/results/get-tb-result-status.php', [
    ...
    {
        self::assertSame([], $this->listed('/tb/results/get-tb-result-status.php', [
            'batchCode' => 'B-ONE%" OR "1"="1',
        ]));
    }
}
