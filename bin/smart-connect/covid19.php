...
// Smart Connect: push pending COVID-19 sample data to the configured upstream API.
...
try {
    ...
    $url = $baseUrl . ($useV2 ? '/api/v2/covid19' : '/api/vlsm-covid19');
    ...
    $result = $apiService->postFile($url, 'covid19File', TEMP_PATH . DIRECTORY_SEPARATOR . $filename, $params, true, true);
    ...
    if (isset($deResult['status']) && trim((string) $deResult['status']) === 'success') {
        $data = [
            'covid19_last_dash_sync' => (empty($lastUpdate) ? DateUtility::getCurrentDateTime() : $lastUpdate)
        ];
