...
// Push outbound Generic referrals (REFERRED status) to the configured partner system.
...
try {
    ...
    //2. if Custom tests module is not active, exit
    if (!TestsService::isTestActive('generic-tests')) {
        echo "Custom Tests module is not active. Exiting script.";
        exit(0);
    }
    ...
    $samplesToUpdate = $db->get('form_generic');
    if (!empty($samplesToUpdate)) {
        foreach ($samplesToUpdate as $sample) {
            $genericId = $sample['sample_id'];
            $oldLabId = $sample['lab_id'];
            $newLabId = $sample['referred_to_lab_id'];

            // Update the form_generic table - set lab_id to referred_to_lab_id
            $updateData = [
                'lab_id' => $newLabId,
            ...
            $db->where("sample_id", $genericId);
            $updated = $db->update('form_generic', $updateData);

            if ($updated) {
                // 4. Insert into generic_referral_history table
                $historyData = [
                ...
                $db->insert('generic_referral_history', $historyData);
            }
        }
    }
} catch (Throwable $e) {
