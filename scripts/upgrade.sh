...
# The off-machine backup runner, for the same reason and from the same code.
#
# /usr/local/bin/intelis-backup.sh is generated by remote-backup.sh on the day
# backups are configured, and until now nothing ever rewrote it. A lab therefore
# kept running whatever runner it was first given: improvements to it shipped in
# release after release and reached nobody, because the only thing that installs
# one is a hand re-run of `intelis backup setup`, and nobody re-runs setup on
# something that is working.
#
# --refresh-runner asks no questions and does nothing at all where no backup is
# configured, so this is safe on every instance including those that have never
# set one up.
backup_setup="${first_lis_path:-${lis_paths[0]}}/scripts/remote-backup.sh"
if [ -f "$backup_setup" ] && [ -f /etc/intelis/backup.conf ]; then
    if bash "$backup_setup" --refresh-runner >>"$log_file" 2>&1; then
        print success "Backup runner refreshed."
        log_action "backup runner refreshed via $backup_setup"
    else
        print warning "Could not refresh the backup runner; the existing one still works. See $log_file."
        log_action "backup runner refresh failed"
    fi
fi
